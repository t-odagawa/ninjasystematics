...
int main(int argc, char* argv[]) {
  ...
  if ( argc != 6 ) {
    BOOST_LOG_TRIVIAL(error) << "Usage : " << argv[0]
			     << " <nominal file name> <detector systematic dir name> <dial name> <hist name> <output file name>";
    return 1;
  }
  ...
  std::string dial_dir_name = std::string(argv[2]);
  std::string dial_name = std::string(argv[3]);

  std::stringstream ss_varied_filename;
  ss_varied_filename << dial_dir_name << "/";
  TString plus_filename;
  plus_filename = ss_varied_filename.str() + "plus/output/" + nominal_path.leaf().string();
  ...
  TFile *minus_file = new TFile(minus_filename, "read");
